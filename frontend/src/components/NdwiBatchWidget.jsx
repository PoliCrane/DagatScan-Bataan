 ...
 * are tracked by NdwiBatchContext, which also resumes the batch (but not
 * single-year, which has no server-side job to resume) after a hard
 * refresh.
 */
export default function NdwiBatchWidget() {
  const { status, running, minimized, dismiss, toggleMinimized, cancelBatch, singleYear } = useNdwiBatch();
